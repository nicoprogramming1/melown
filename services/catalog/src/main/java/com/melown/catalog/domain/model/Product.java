...
public class Product {
    ...
    public static Product createNew(NewProductSpec spec) {
        ...
        return new Product(productId, spec.name(), spec.description(), spec.specifications(), spec.vendorId(), productCondition, productStatus, spec.version(), Instant.now(), Instant.now());
    }
    ...
    public static Product createUsed(UsedProductSpec spec) {
        ...
        ProductCondition productCondition = new UsedCondition(spec.usedDescription(), spec.conditionGrade());
    ...
    public static Product rehydrate(ProductSnapshot snapshot) {
        ...
        return new Product(productId, snapshot.name(), snapshot.description(), snapshot.specifications(), snapshot.vendorId(), snapshot.productCondition(), snapshot.productStatus(), snapshot.version(), snapshot.createdAt(), snapshot.updatedAt());
    }

    public ProductSnapshot toSnapshot() {
        return new ProductSnapshot(id, name, description, specifications, vendorId, productCondition, productStatus, version, createdAt, updatedAt);
    }
}