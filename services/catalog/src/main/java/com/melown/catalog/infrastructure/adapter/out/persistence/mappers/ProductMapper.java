...
public class ProductMapper {
    ...
    static Product toDomain(ProductEntity productEntity) {

        Specifications specifications = new Specifications(Map.copyOf(productEntity.getSpecifications()));
        ...
        ProductCondition productCondition =
                ...
                switch (productEntity.getConditionKind()) {
                    ...
                    case USED ->
                            new UsedCondition(productEntity.getUsedDescription(), productEntity.getUsedConditionGrade());
                };

        ProductStatus productStatus = switch (productEntity.getStatusKind()) {
            ...
            case ARCHIVED -> new ArchivedStatus(productEntity.getArchivedAt(), productEntity.getReason());
        };

        ProductId productId = ProductId.reconstitute(productEntity.getId());

        ProductSnapshot snapshot = new ProductSnapshot(productId, productEntity.getName(), productEntity.getDescription(), specifications, // aun tengo las Specifications incompletas
                productEntity.getVendorId(), productCondition, productStatus, productEntity.getVersion(), productEntity.getCreatedAt(), productEntity.getUpdatedAt());

        return Product.rehydrate(snapshot);
    }

    static ProductEntity toEntity(ProductSnapshot snapshot) {
